...
def ui() -> None:
    ...
    gem_l = Label(root, text="Gemini API token", font="Times 11 bold", justify=LEFT)
    gem_l.pack()
    gem_e = Entry(root, bd=5)
    gem_e.pack()
    gem_b = tk.Button(
        root, text="", command=lambda: gemini_token_set(gem_e.get(), gem_e, gem_b, gem_done_l))
    gem_b.pack(pady=5)
    gem_done_l = Label(root, text="")
    gem_done_l.pack()
    ...
    db_and_co()
    tokens_check(bft_b)
    gemini_token_check(gem_b)
    root.mainloop()


def gemini_token_check(btn: Any=None) -> None:
    if not db.token_get("Gemini_token"):
        if btn:
            btn.configure(text="Confirm")
    else:
        if btn:
...
def gemini_token_set(token: str, entry: Any=None, btn: Any=None, lbl: Any=None) -> None:
    if token:
        db.token_set("Gemini_token", token)
        text = "Token saved!"
        if entry and btn and lbl:
            entry.destroy()
            btn.destroy()
...
def parse_args() -> bool:
    ...
    group.add_argument("--gemini_token", help="Gemini API token", metavar="Gemini_token")
    ...
    elif args.gemini_token:
        gemini_token_set(args.gemini_token)
    elif args.start:
        bot_start()
    elif args.admin_add:
