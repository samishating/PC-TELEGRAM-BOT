...
engine = create_engine("sqlite:///" + database(), echo=False, poolclass=NullPool)
...
def update_user(from_user: User, bot: Bot) -> None:  # Update the user list (db)
    ...
    if user:
        ...
        user.last_use = datetime.now(get_localzone()).strftime('%Y-%m-%d %H:%M')
        ...
        if session.query(Users).count() == 0:
            new_user = Users(name_first=from_user.first_name,
                             ...
                             last_use=datetime.now(get_localzone()).strftime('%Y-%m-%d %H:%M'),
                             time_used=used + 1,
                             id=from_user.id,
                             privs="-2")
        else:
...
def admin_check(func: Callable) -> Callable[[Update, CallbackContext], Optional[Callable]]:
    ...
    def is_admin(update: Update, context: CallbackContext) -> Optional[Callable]:
        session = DBsession()
        try:
            user = session.query(Users).filter(Users.id == update.message.from_user.id).one_or_none()
            privs = user.privs if user else None
        finally:
            session.close()
